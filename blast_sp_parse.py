...
with open(sys.argv[1]+".xml", "w") as save_file:
...
with open(sys.argv[1]+".xml") as p:
    print("Parsing blast output")
    blast_recs=NCBIXML.parse(p)
    count = 0
    for rec in blast_recs:
        count += 1
        print(float(count)/float(numqueries))
...
for tc_id in error_dic:
    count += 1
    print(float(count)/float(len(error_dic)))
    print(tc_id)
    list_of_GIs = []
